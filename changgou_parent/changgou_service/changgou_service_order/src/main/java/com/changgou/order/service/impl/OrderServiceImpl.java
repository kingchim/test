...
import com.changgou.order.dao.TaskHistMapper;
...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private TaskHistMapper taskHistMapper;
    ...
    public String add(Order order) {
        ...
        order.setOrderStatus("0");//0，未完成，1，已完成，2，退货
        order.setPayStatus("0");//"0",未支付，"1"，已支付
        order.setConsignStatus("0");//"0",未发货，"1"，已发货
        String orderId = idWorker.nextId() + "";
        order.setId(orderId);
        orderMapper.insertSelective(order);
        //填充订单数据并保存到tb_order_item
        for (OrderItem orderItem : orderItemList) {
            orderItem.setId(idWorker.nextId() + "");
            orderItem.setIsReturn("0");//0未退货，1已退货
